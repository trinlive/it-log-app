/**
 * main.js (Updated: Grouping Chart Logic - Merge Server+Dev, Service->Other)
 */
...
// 🛠️ ฟังก์ชันช่วยจัดกลุ่ม (Grouping Helper) - ตรงกับ Controller
function getCategoryGroup(categoryLabelCode) {
    ...
    // 5. Other (รวม Meeting/Service และอื่นๆ)
    return 'Other (อื่นๆ)';
}
...
function updateDashboard(visibleRows) {
    ...
    visibleRows.forEach(row => {
        ...
        // ดึงค่า Label Code (helpdesk.computer, permission.wifi ฯลฯ)
        ...
        if (['closed', 'เสร็จสิ้น', 'เรียบร้อย'].includes(status)) {
            ...
            // ถ้าเป็น 'fix' ไม่นับเป็น Active (ไปรวมใน Fix)
            if (status !== 'fix') {
                countActive++;
            }
        }
